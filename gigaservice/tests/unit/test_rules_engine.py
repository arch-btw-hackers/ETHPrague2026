...
class TestRulesEngineHappyPath:
    def test_all_within_limits_is_valid(self):
        readings = {"temp_c": 20.0, "acceleration_x": 1.0, "acceleration_y": 0.5}
    ...
    def test_exact_limit_values_are_valid(self):
        ...
        readings = {"temp_c": 25.0, "acceleration_x": 2.0, "acceleration_y": 2.0}
        assert evaluate_conditions(readings, CONDITIONS) is True

    def test_zero_readings_valid(self):
        readings = {"temp_c": 0.0, "acceleration_x": 0.0, "acceleration_y": 0.0}
        assert evaluate_conditions(readings, CONDITIONS) is True

    def test_negative_acceleration_within_limit(self):
        # abs() check — negative values should use absolute value
        readings = {"temp_c": 10.0, "acceleration_x": -1.5, "acceleration_y": -1.0}
        assert evaluate_conditions(readings, CONDITIONS) is True
...
class TestTemperatureViolation:
    def test_temp_exceeds_max(self):
        readings = {"temp_c": 26.0, "acceleration_x": 0.0, "acceleration_y": 0.0}
        assert evaluate_conditions(readings, CONDITIONS) is False

    def test_temp_just_above_limit(self):
        readings = {"temp_c": 25.001, "acceleration_x": 0.0, "acceleration_y": 0.0}
        assert evaluate_conditions(readings, CONDITIONS) is False

    def test_very_high_temp(self):
        readings = {"temp_c": 999.0, "acceleration_x": 0.0, "acceleration_y": 0.0}
        assert evaluate_conditions(readings, CONDITIONS) is False

    def test_negative_temp_always_valid(self):
        readings = {"temp_c": -40.0, "acceleration_x": 0.0, "acceleration_y": 0.0}
        assert evaluate_conditions(readings, CONDITIONS) is True
...
class TestAccelerationViolation:
    def test_acc_x_exceeds_max(self):
        readings = {"temp_c": 10.0, "acceleration_x": 3.0, "acceleration_y": 0.0}
        assert evaluate_conditions(readings, CONDITIONS) is False

    def test_acc_y_exceeds_max(self):
        readings = {"temp_c": 10.0, "acceleration_x": 0.0, "acceleration_y": 3.0}
        assert evaluate_conditions(readings, CONDITIONS) is False

    def test_negative_acc_x_exceeds_max(self):
        readings = {"temp_c": 10.0, "acceleration_x": -3.0, "acceleration_y": 0.0}
        assert evaluate_conditions(readings, CONDITIONS) is False

    def test_negative_acc_y_exceeds_max(self):
        readings = {"temp_c": 10.0, "acceleration_x": 0.0, "acceleration_y": -3.0}
        assert evaluate_conditions(readings, CONDITIONS) is False

    def test_acc_just_above_limit(self):
        readings = {"temp_c": 10.0, "acceleration_x": 2.001, "acceleration_y": 0.0}
        assert evaluate_conditions(readings, CONDITIONS) is False
...
class TestMultipleViolations:
    def test_all_three_violated(self):
        readings = {"temp_c": 30.0, "acceleration_x": 5.0, "acceleration_y": 5.0}
        assert evaluate_conditions(readings, CONDITIONS) is False

    def test_temp_and_acc_x_violated(self):
        readings = {"temp_c": 30.0, "acceleration_x": 5.0, "acceleration_y": 0.0}
        assert evaluate_conditions(readings, CONDITIONS) is False
...
@pytest.mark.parametrize("temp,acc_x,acc_y,expected", [
    (25.0,  2.0,  2.0,  True),   # exact boundary — valid
    (25.01, 2.0,  2.0,  False),  # 1 tick over temp
    (25.0,  2.01, 2.0,  False),  # 1 tick over acc_x
    (25.0,  2.0,  2.01, False),  # 1 tick over acc_y
    (-99.0, 0.0,  0.0,  True),   # extreme cold
    (0.0,  -2.0, -2.0,  True),   # exact negative boundary
    (0.0,  -2.01, 0.0,  False),  # just over negative boundary
])
def test_boundary_parametrized(temp, acc_x, acc_y, expected):
    readings = {"temp_c": temp, "acceleration_x": acc_x, "acceleration_y": acc_y}
...
class TestCustomConditions:
    def test_strict_conditions(self):
        ...
        readings = {"temp_c": 5.1, "acceleration_x": 0.1, "acceleration_y": 0.1}
    ...
    def test_relaxed_conditions(self):
        ...
        readings = {"temp_c": 80.0, "acceleration_x": 30.0, "acceleration_y": 30.0}
    ...
    def test_zero_max_acceleration(self):
        ...
        readings = {"temp_c": 10.0, "acceleration_x": 0.001, "acceleration_y": 0.0}
